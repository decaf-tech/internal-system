...
function revalidateNote(noteId: string) {
  revalidatePath("/backoffice/notes");
  revalidatePath(`/backoffice/notes/${noteId}`);
  ...
  revalidatePath("/backoffice/tasks");
}
...
export async function createNote(input: NewNoteInput = {}) {
  ...
  redirect(`/backoffice/notes/${created.id}`);
}
...
export async function openMeetingNote(input: {
  ...
  if (existing) redirect(`/backoffice/notes/${existing.note_id}`);

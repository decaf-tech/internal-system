...
export default async function ClientsPipelinePage() {
  ...
  const [prospectsResult, membersResult, userResult, stats, requestsResult] =
    await Promise.all([
      supabase
        .from("prospects")
      ...
      // Isian form situs publik yang belum ditindaklanjuti siapa pun.
      supabase
  ...
  const requests = (requestsResult.data ?? []) as DiscoveryRequest[];
  ...
  return (
    <>
      ...
      <ClientTabs active="/backoffice/clients" />
      <ClientStatStrip stats={stats} />

      <DiscoveryInbox requests={requests} />

...
/**
 * Prospek muncul di dua rute: papannya di `/backoffice/clients`, dan tiga dari empat
 * angka strip statistik (prospek aktif, nilai pipeline, follow-up jatuh
 * tempo) ikut tergambar di `/backoffice/clients/list`. Sama seperti `revalidateClientPages`
...
export async function markProspectWon(
  ...
  revalidateProspectPages();
  revalidatePath(`/backoffice/clients/${prospect.client_id}`);
